...
void perm(int list[], int k, int m, int* p, Mat* mat, double* det) {
    ...
    if (k > m) {
        ...
        if (!equal(mat->element[k][list[k]], 0.0))
            perm(list, k + 1, m, p, mat, det);
        for (i = k + 1; i <= m; i++) {
            if (equal(mat->element[k][list[i]], 0.0))
                continue;
            swap(&list[k], &list[i]);
            *p += 1;
            perm(list, k + 1, m, p, mat, det);
...
Mat* MatCreate(Mat* mat, int row, int col) {
    ...
    mat->element = (double**)malloc((size_t)row * sizeof(double*));
    if (mat->element == NULL) {
        printf("mat create fail!\n");
        return NULL;
    }
    for (i = 0; i < row; i++) {
        mat->element[i] = (double*)malloc((size_t)col * sizeof(double));
...
void MatDump(const Mat* mat) {
    ...
    for (row = 0; row < mat->row; row++) {
        for (col = 0; col < mat->col; col++) {
            printf("%.4lf\t", mat->element[row][col]);
        }
        printf("\n");
    }
}
...
double MatDet(Mat* mat) {
    ...
    list = (int*)malloc(sizeof(int) * (size_t)mat->col);
    if (list == NULL) {
